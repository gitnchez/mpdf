		...
		switch ($tag) {
			...
			case 'MAIN':
				...
				if (isset($properties['FLOAT']) && strtoupper($properties['FLOAT']) == 'RIGHT' && !$this->mpdf->ColActive) {
					...
					list($l_exists, $r_exists, $l_max, $r_max, $l_width, $r_width) = $this->mpdf->GetFloatDivInfo($this->mpdf->blklvl - 1);
					$maxw = $container_w - $l_width - $r_width;
					if (((float)$setwidth + (float)$currblk['margin_left'] + (float)$currblk['margin_right'] + (float)$bdl + (float)$pdl + (float)$bdr + (float)$pdr) > (float)$maxw || ((float)$maxw - ((float)$currblk['margin_right'] + (float)$currblk['margin_left'] + (float)$bdl + (float)$pdl + (float)$bdr + (float)$pdr)) < (3 * (float)$this->mpdf->GetCharWidth('W', false))) {
						// Too narrow to fit - try to move down past L or R float
